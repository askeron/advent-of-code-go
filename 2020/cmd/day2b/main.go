...
func isValid(line string) bool {
	...
	matcher := regex.FindStringSubmatch(line)
	min, _ := strconv.Atoi(matcher[1])
	max, _ := strconv.Atoi(matcher[2])
	ruleRune := getRune(matcher[3], 0)
	password := matcher[4]

	return (ruleRune == getRune(password, min-1)) != (ruleRune == getRune(password, max-1))
}

func getRune(str string, index int) (r rune) {
	i := 0
	for i, r = range str {
		if i == index {
			return
